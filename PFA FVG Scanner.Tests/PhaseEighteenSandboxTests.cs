...
public sealed class PhaseEighteenSandboxTests
{
    ...
    public async Task InstrumentEconomicsSlippageAndCommissionsFlowToPerformance()
    {
        ...
        await service.SubmitSignalAsync("buy","A",Signal("I","BUY",Now,quantity:2),model,Token);clock.UtcNow=Now.AddMinutes(1);await service.ProcessMarketAsync("buy-fill","A",Market("B",100,101,99,100,2,clock.UtcNow),model,Token);
        await service.SubmitSignalAsync("sell","A",Signal("I","SELL",clock.UtcNow,quantity:2,side:SandboxOrderSide.Sell),model,Token);clock.UtcNow=Now.AddMinutes(2);var state=await service.ProcessMarketAsync("sell-fill","A",Market("S",102,103,101,102,2,clock.UtcNow),model,Token);
