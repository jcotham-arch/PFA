...
async function hydrate(){
  ...
  if(patternTradeResponse.ok){
    const runs=await patternTradeResponse.json();if(runs.length){const run=runs[0];document.getElementById('patternTradeState').textContent='EVALUATED';document.getElementById('patternTradeMeta').textContent=`${number(run.observationCount)} observations · ${number(run.hypothesisCount)} hypotheses · ${number(run.sampleCount)} cost-adjusted samples`;const validation=run.summaries.filter(x=>x.split==='Validation').sort((a,b)=>Number(b.meanNetR)-Number(a.meanNetR)).slice(0,10);document.getElementById('patternTradeRows').innerHTML=validation.map(metric=>{const test=run.summaries.find(x=>x.hypothesisId===metric.hypothesisId&&x.split==='Test');return `<tr><td>${esc(metric.moduleId)}</td><td>${esc(metric.entryPolicy)} · ${esc(metric.stopPolicy)} · ${esc(metric.directionPolicy)}</td><td>${number(metric.targetR)}R / ${number(metric.maximumHoldingMinutes)}m</td><td>${Number(metric.meanNetR).toFixed(3)}R</td><td>${test?Number(test.meanNetR).toFixed(3)+'R':'—'}</td></tr>`}).join('');}
  }
}
