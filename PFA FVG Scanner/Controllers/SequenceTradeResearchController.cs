...
[ApiController]
[Route("api/research/sequence-trades")]
public sealed class SequenceTradeResearchController(SequenceTradeResearchService service):ControllerBase
{
    [HttpGet]
    public async Task<IActionResult> Get(CancellationToken token)=>Ok(await service.GetAllAsync(token));

    [HttpPost]
    public async Task<IActionResult> Run([FromBody] SequenceTradeResearchRequest request,CancellationToken token)
    {
